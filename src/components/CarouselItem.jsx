...
export const CarouselItem = ({ title, benef, principiosactivos, mododeuso, desc, img }) => {
  ...
  return (
    <div className='duration-150 ease-linear'>
      <img
      ...
      <div className='absolute hidden lg:block w-1/2 -translate-x-1/2 -translate-y-1/2 top-1/2 -right-1/4 mr-24'>
        <article className='flex flex-col bg-[#F5F5F5]/70 p-3 px-5 rounded-xl'>
          {Check(title)
            ? (
      ...
      <div className='absolute block lg:hidden w-full top-full z-20'>
        <article className='flex flex-col bg-[#F5F5F5] p-3 sm:px-7'>
          {Check(title)
            ? (
